const {send, receive} = ipcRenderer;

// outgoing events
document.querySelector('form').addEventListener('submit',function(e){e.preventDefault();});
document.getElementById("outputButton").addEventListener('click', function(e){e.preventDefault(); send('button:click', 'outputDir');});
document.getElementById("edlButton").addEventListener('click', function(e){e.preventDefault(); send('button:click', 'edlFile');});
document.getElementById("mergeButton").addEventListener('click', function(e){e.preventDefault(); send('button:click', 'mergeFile');});
...
receive('edlFilters:value', function(vals){
	...
	//table.innerHTML = "";
	vals.forEach(function(val){ console.log(val);
		...
		const row = rows[rows.length - 1]; console.log(row);
		...
		row.querySelector('.filter-end').innerText = val.end;
...
vidFileDragDrop.addEventListener('click', function(e){e.preventDefault(); send('button:click', 'videoFile');});
vidFileDragDrop.addEventListener('dragover', function(e){e.preventDefault();});
